...
jlcc <- function(gip){
  l.length <- length(gip)
  for (i in 1:l.length) {
    gip[[i]] <- delete.edges(gip[[i]], which(E(gip[[i]])$weight==0))
  }
  #find joint largest connected component
  df1 <- igraph::as_data_frame(gip[[1]])[,1:2]
  df2 <- igraph::as_data_frame(gip[[2]])[,1:2]
  g <- graph.intersection(gip[[1]],gip[[2]],keep.all.vertices = TRUE)
  if(l.length>2){
    for (i in 3:l.length) {
      g <- graph.intersection(g,gip[[i]],keep.all.vertices = TRUE)
    }
  }
  ...
  gip <- list()
  for (i in 1:l.length) {
    gip[[i]] <- induced_subgraph(glist[[i]], V(lcc)$name);
    gip[[i]] <- permute.vertices(gip[[i]], match(V(gip[[i]])$name, V(gip[[1]])$name));
  }
  for (i in 1:l.length) {
    gip[[i]] <- simplify(gip[[i]])
  }
  return(gip)
}
...
pltclique <- function(gip, p, art.anomaly.v){
  ...
  art.gip <- gip
  adj <- as.matrix(get.adjacency(art.gip[[p]], attr = attrweigth))
  adj[middle.max.inx,middle.max.inx] <- (adj[middle.max.inx,middle.max.inx] + 1)
  art.gip[[p]] <- graph.adjacency(adj,mode = "undirected", weighted = TRUE,diag = FALSE)
...
getweightchange <- function(gip){
  ...
  for (i in 1:l.length) {
    num.edge.weight[,i] <- graph.strength(gip[[i]],loops = FALSE)
  }
